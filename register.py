...
class Register:
    def __init__(self,root):
        ...
        self.bg_mainscreen = PhotoImage(file="bg_register.png")
    ...
    #BUTTONS
        #save
        img_save=Image.open(r"pictures\save.png")
        ...
        #update
        img_update=Image.open(r"pictures\update.png")
        ...
        #delete
        img_delete=Image.open(r"pictures\delete.png")
        ...
        #clear
        img_clear=Image.open(r"pictures\clear.png")
        ...
        #take_photo
        img_take_photo=Image.open(r"pictures\take_photo.png")
        ...
        #photos
        img_photos_inactive=Image.open(r"pictures\photos_inactive.png")
        img_photos_inactive=img_photos_inactive.resize((70,70))

        img_photos_active=Image.open(r"pictures\photos_active.png")
        ...
        #train
        img_train_inactive=Image.open(r"pictures\train_inactive.png")
        img_train_inactive=img_train_inactive.resize((70,70))

        img_train_active=Image.open(r"pictures\train_active.png")
        ...
        img_home_inactive=Image.open(r"pictures\home_inactive.png")
        img_home_inactive=img_home_inactive.resize((70,70))

        img_home_active=Image.open(r"pictures\home_active.png")
    ...
    #DATABASE TABLE
        ...
        self.student_table=ttk.Treeview(frame_DB,columns=("ID","Last Name","First Name","Middle Initial","Gender","E-mail","Phone No.", "Address","Photo","Employee ID"),xscrollcommand=hor_scroll.set, yscrollcommand=ver_scroll.set)
    ...
    def open_photos(self):
        ...
        self.new_window.iconbitmap('attentiface_icon.ico')
    ...
    def train_classifier(self):
        validation = messagebox.askyesno("Attentiface V 1.0","Are you sure you want to register the data?",parent=self.root)
        if validation > 0:
    ...
    def add(self):
        if self.var_Name.get()=="" or self.var_ID.get()=="":
            ...
            try:
                ...
                conn.commit()
                self.show_data()
                messagebox.showinfo("Attentiface V 1.0","Input successful",parent=self.root)
    ...
    def update_data(self):
        if self.var_Name.get()=="" or self.var_ID.get()=="":
            ...
            try:
                Update=messagebox.askyesno("Attentiface V 1.0", "Are you sure you want to update this entry?",parent=self.root)
                if Update>0:
                    conn = sqlite3.connect("face_recognition_software_system")
                    cursor = conn.cursor()
                    ...
                    if not Update:
                        return
                messagebox.showinfo("Attentiface V 1.0","Data updated successfully",parent=self.root)
                conn.commit()
                self.show_data()
                conn.close()
            except Exception as es:
    ...
    def delete(self):
        if self.var_ID.get()=="":
            messagebox.showerror("Attentiface V 1.0","Select an entry to delete",parent=self.root)
        else:
            try:
                delete=messagebox.askyesno("Attentiface V 1.0","Are you sure you want to delete this data?",parent=self.root)
                if delete>0:
                    conn = sqlite3.connect("face_recognition_software_system")
                    cursor = conn.cursor()
    ...
    def display(self):
        ...
        #train
        img_train1=Image.open(r"pictures\start.png")
        img_train1=img_train1.resize((120,60))

        self.new_window.img_train1=ImageTk.PhotoImage(img_train1)
        button_train=Button(self.new_window,image=self.new_window.img_train1,bg="white",borderwidth=0,width=200,highlightthickness=0,height=200,bd=0,relief="raised",activebackground="white",command=self.take_photo_data)
    ...
    def take_photo_data(self):
        if self.var_Name.get()=="" or self.var_ID.get()=="" or self.var_keyID=="":
            messagebox.showerror("Attentiface V 1.0","Please fill-in the fields first",parent=self.root)
        else:
            try:
                take_pic=messagebox.askyesno("Attentiface V 1.0", "Are you sure you want to take a photo?",parent=self.root)
                if take_pic>0:
                    ...
                    classifer_model=cv2.CascadeClassifier("haarcascade_frontalface_default.xml")
                    ...
                    messagebox.showinfo("Attentiface V 1.0","All photo samples taken successfully",parent=self.root)
                else:
                    if not take_pic:
                        return
            except Exception as es:
    ...
    def search(self):
        if self.var_search.get()=="" or self.var_search_entry.get()=="":
            messagebox.showerror("Attentiface V 1.0","Please fill in the search details")
        else:
            try:
               conn = sqlite3.connect("face_recognition_software_system")
               cursor = conn.cursor()
